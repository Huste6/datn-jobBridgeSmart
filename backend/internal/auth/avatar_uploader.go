...
func NewAvatarUploader(cloudinaryURL, folder string) (*AvatarUploader, error) {
...
func (u *AvatarUploader) Enabled() bool {
	return u != nil
}

func (u *AvatarUploader) UploadImage(ctx context.Context, userID string, filename string, data []byte) (string, error) {
	timestamp := time.Now().Unix()
	publicID := fmt.Sprintf("user_%s_%d", userID, timestamp)
...
func (u *AvatarUploader) sign(timestamp int64, publicID string) string {
